...
| Refined types | Iron / iron-cats | **3.0.2** | Compile scope, `core` module. **Deviation from design.md's "2.6.x"**: 3.0.2 is the latest on Maven Central as of 2026-06-13; used directly since this is a new project with no 2.x migration cost. Flagged for the Step 1 human gate — Iron 3.x API differences (if any) surface in the typed contract. **No iron-scalacheck/iron-hedgehog**: Hedgehog has no `Arbitrary` typeclass, so refined-type generators are hand-written (generate raw value → refine via the smart constructor's `.either`/`applyUnsafe`). |
...
| WartRemover | **NOT AVAILABLE** | `sbt-wartremover` 3.2.3 (latest, Oct 2024) requires `org.wartremover:wartremover_3.8.4`, which does not exist on Maven Central — no wartremover build targets Scala 3.8.4 yet. Plugin removed from `project/plugins.sbt` (commented rationale left in place). Scalafix `DisableSyntax` above covers the syntactic rules (var/null/throw/return/casts) that the planned wart set would have enforced; `.get`-style partial-extraction warts (`OptionPartial`/`EitherProjectionPartial`/`TryPartial`) have no Scalafix equivalent and are covered only by Ring 8 review until wartremover publishes a 3.8.x build | `project/plugins.sbt` |
...
compiler warnings-as-errors; WartRemover is degraded per above. Ring 2
(architecture) has no automated rule yet and is enforced by review against
the design's layer table (no Scalafix `LayerDependencies` rule configured).
...
| 1 Lint | ✅ (degraded) | Scalafix DisableSyntax/RemoveUnused/OrganizeImports + scalafmt active and verified; WartRemover unavailable (Scala 3.8.4 — see Static Analysis table) |
